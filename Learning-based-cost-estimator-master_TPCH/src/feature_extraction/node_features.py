...
def extract_info_from_node(node, alias2table):
    ...
    if 'Plan Rows' in node:
        input_cardinality = node['Plan Rows']
    if 'Actual Rows' in node:
        output_cardinality = node['Actual Rows']
    ...
    if node['Node Type'] == 'Materialize':
        ...
        return Hash(input_cardinality, output_cardinality*loops, width*input_cardinality), None
    elif node['Node Type'] == 'Sort':
        keys = [change_alias2table(key, alias2table) for key in node['Sort Key']]
        return Sort(keys, input_cardinality, output_cardinality*loops, width*input_cardinality), None
    elif node['Node Type'] == 'BitmapAnd':
        return BitmapCombine('BitmapAnd', input_cardinality, output_cardinality*loops, width*input_cardinality), None
    elif node['Node Type'] == 'BitmapOr':
        return BitmapCombine('BitmapOr', input_cardinality, output_cardinality*loops, width*input_cardinality), None
    elif node['Node Type'] == 'Result':
        return Result(), None
    elif node['Node Type'] == 'Hash Join':
        return Join('Hash Join', pre2seq(node["Hash Cond"], alias2table, relation_name, index_name), input_cardinality, output_cardinality*loops, width*input_cardinality), None
    elif node['Node Type'] == 'Merge Join':
        return Join('Merge Join', pre2seq(node["Merge Cond"], alias2table, relation_name, index_name), input_cardinality, output_cardinality*loops, width*input_cardinality), None
    elif node['Node Type'] == 'Nested Loop':
        if 'Join Filter' in node:
            condition = pre2seq(node['Join Filter'], alias2table, relation_name, index_name)
        else:
            condition = []
        return Join('Nested Loop', condition, input_cardinality, output_cardinality*loops, width*input_cardinality), None
    elif node['Node Type'] == 'Aggregate':
        if 'Group Key' in node:
            keys = [change_alias2table(key, alias2table) for key in node['Group Key']]
        else:
            keys = []
        return Aggregate(node['Strategy'], keys, input_cardinality, output_cardinality*loops, width*input_cardinality), None
        ...
        return Scan('Seq Scan', condition_seq_filter, condition_seq_index, relation_name, index_name, input_cardinality, output_cardinality*loops, width*input_cardinality), None
        ...
        if 'Filter' in node:
            ...
            condition_seq_filter = []
        condition_seq_index, relation_name, index_name = [], node["Relation Name"], None
        return Scan('Bitmap Heap Scan', condition_seq_filter, condition_seq_index, relation_name, index_name, input_cardinality, output_cardinality*loops, width*input_cardinality), None
        ...
        if len(condition_seq_index) == 1 and re.match(r'[a-zA-Z]+', condition_seq_index[0].right_value) is not None:
            return Scan('Index Scan', condition_seq_filter, condition_seq_index, relation_name,
                        index_name, input_cardinality, output_cardinality*loops, width*input_cardinality), condition_seq_index
        else:
            return Scan('Index Scan', condition_seq_filter, condition_seq_index, relation_name, index_name, input_cardinality, output_cardinality*loops, width*input_cardinality), None
        ...
        if len(condition_seq_index) == 1 and re.match(r'[a-zA-Z]+', condition_seq_index[0].right_value) is not None:
            ...
            return Scan('Bitmap Index Scan', condition_seq_filter, condition_seq_index, relation_name, index_name, input_cardinality, output_cardinality*loops, width*input_cardinality), None
        ...
        if len(condition_seq_index) == 1 and re.match(r'[a-zA-Z]+', condition_seq_index[0].right_value) is not None:
            ...
            return Scan('Index Only Scan', condition_seq_filter, condition_seq_index, relation_name, index_name, input_cardinality, output_cardinality*loops, width*input_cardinality), None
    else:
