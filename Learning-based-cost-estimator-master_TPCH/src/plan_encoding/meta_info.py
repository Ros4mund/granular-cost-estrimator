...
def determine_prefix(column):
    ...
    if relation_name == 'customer':
        if column_name == 'c_custkey':
            return 'c_custkey_'
        else:
            print (column)
            raise
    elif relation_name == 'lineitem':
        if column_name == 'l_orderkey':
            return 'l_orderkey_'
        else:
            print (column)
            raise
        ...
        if column_name == 'o_orderkey':
            return 'o_orderkey_'
        else:
            print (column)
            raise
    elif relation_name == 'part':
        if column_name == 'p_partkey':
            return 'p_partkey_'
        else:
            print (column)
            raise
    elif relation_name == 'partsupp':
        if column_name == 'ps_partkey':
            return 'ps_partkey_'
        else:
            print (column)
            raise
    elif relation_name == 'region':
        if column_name == 'r_regionkey':
            return 'r_regionkey_'
        else:
            print (column)
            raise
    elif relation_name == 'supplier':
        if column_name == 's_suppkey':
            return 's_suppkey_'
        else:
            print (column)
            raise
...
def prepare_dataset(database):
    ...
    tables = ['call_center', 'catalog_page', 'catalog_returns', 'catalog_sales', 'customer', 'customer_address', 'customer_demographics',
    ...
    for table_name in tables:
        column2pos[table_name] = database[table_name]

    indexes = ['cc_call_center_sk', 'cc_call_center_id', 'cc_rec_start_date','cc_rec_end_date','cc_closed_date_sk',
               'cc_open_date_sk','cc_name','cc_class','cc_employees','cc_sq_ft','cc_hours','cc_manager','cc_mkt_id',
               'cc_mkt_class','cc_mkt_desc','cc_market_manager','cc_division','cc_division_name','cc_company','cc_company_name',
               'cc_street_number','cc_street_name','cc_street_type','cc_suite_number','cc_city','cc_county','cc_state',
               'cc_zip','cc_country','cc_gmt_offset','cc_tax_percentage','cp_catalog_page_sk','cp_catalog_page_id','cp_start_date_sk','cp_end_date_sk',
               'cp_department', 'cp_catalog_number','cp_catalog_page_number','cp_description','cp_type', 'cr_returned_date_sk','cr_returned_date_sk','cr_returned_time_sk',
           'cr_item_sk',
